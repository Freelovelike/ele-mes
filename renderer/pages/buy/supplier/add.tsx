...
import { useCreateSupplierMutation } from "../../../src/generated/graphql"
...
const addLocation: NextPage<addProps> = () => {
  ...
  const CreateItemFunc = async (
    ...
    if (result?.data?.createSupplier?.data) {
      success("创建成功")
      if (other) {
        ...
        router.push({
          pathname: `/buy/supplier/${result?.data?.createSupplier?.data?.id}/change`,
        })
      } else {
      ...
      error(result.data.createSupplier.errors[0].message)
    }
  }
  return (
    <Container>
