...
import { useCreateLocationMutation } from "../../../src/generated/graphql"
...
const addLocation: NextPage<addProps> = () => {
  ...
  const [{ fetching: createLocationFetching }, createLocation] =
  ...
  const CreateItemFunc = async (
    ...
    if (result.data.createLocation.errors) {
      error(result.data.createLocation.errors[0].message)
    } else {
      success("创建成功")
      if (other) {
        ...
        router.push({
          pathname: `/sales/location/${result?.data?.createLocation?.data?.id}/change`,
        })
      } else {
        router.push({ pathname: "/sales/location", query: {} })
      }
    }
  }
  return (
