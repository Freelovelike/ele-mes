...
const location: NextPage<locationProps> = () => {
  ...
  const [{ fetching: deleteItemFetching }, deleteItems] =
    useDeletelocationsMutation()
  const [selectData, setSelectData] = useState<Location[]>([])
  const [updateItemQueen, setUpdateItemQueen] = useState<Location[]>([])
  ...
  const select = (item?: Location) => {
    ...
    //全选，没传就默认全选
    return () => {
      selectData.length === data?.locations?.total
        ? setSelectData([])
        : setSelectData(data?.locations?.data)
    }
  }
  const DelItem = () => {
    DeleteConfirm(async () => {
    ...
    <ContentInfo>
      <Header>
        <div className='Location'>
          <span style={{ marginRight: "15px" }}>地点</span>
        </div>
        <div className='btn'>
          ...
          <div className='operation_btn operationSeconde'>
            <BlueBtn
              canClick={true}
              content='add'
              onClick={() => router.push("/sales/location/add")}
            />
            <BlueBtn
      ...
      >
        {data?.locations?.data?.length > 0 ? (
          <table cellSpacing={0} cellPadding={0}>
            ...
            <tbody>
              {data?.locations?.data?.map((item, _index) => {
                return (
                  <tr
                    key={item.id}
                    style={{
                  ...
                  >
                    ...
                    <TD>
                      <div
                        ...
                        onClick={() =>
                          router.push({
                            pathname: `/sales/location/${item.id}/detail`,
                          })
                        }
                      >
                        {item.name}
