...
import { useCreateCustomerMutation } from "../../../src/generated/graphql"
...
const add: NextPage<addProps> = () => {
  ...
  const [{ fetching: createItemFetching }, createCustomer] =
  ...
  const CreateItemFunc = async (
    ...
    if (result.data.createCustomer.errors) {
      error(result.data.createCustomer.errors[0].message)
    } else {
      success("创建成功")
      if (other) {
        ...
        router.push({
          pathname: `/sales/customer/${result?.data?.createCustomer?.data?.id}/change`,
        })
      } else {
        router.push({ pathname: "/sales/customer", query: {} })
      }
    }
  }
  return (
