...
const index: NextPage<indexProps> = () => {
  ...
  const [{ data: operationArr }] = useCatchNodeFromResourceQuery({
    variables: { resourceName: name },
  })
  ...
  useEffect(() => {
    const arr: AllNodes[] = JSON.parse(
      operationArr?.catchNodeFromResource ?? "[]"
    ...
    <PathContainer>
      <div>
        <div className='paddingRole'>
          生产资料 {"resource"} @ {router.query.name || "加载中..."}:供应路径
        </div>
        <Menu
          detailRouterPath='back'
          supplierRouterPath='/supplypath/resource/${router.query.name}'
        />
      </div>
      <ContainerViewer>
        <div
          ...
          {operationArr ? (
            <NoSSRForceGraph
              ...
              nodeCanvasObject={(node: newNode, ctx) => {
                ...
                ctx.font = "normal 5px sans-serif"
              }}
            />
          ) : (
            "loading..."
